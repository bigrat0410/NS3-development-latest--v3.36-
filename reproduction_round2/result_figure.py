...
def main():
    ...
    parser.add_argument("--reinrate-glob")
    ...
    if args.ideal_glob and args.minstrel_glob:
        ...
        reinrate_runs = 0
        if args.reinrate_glob:
            rx, ry, reinrate_runs = average_files(args.reinrate_glob)
            series.append((args.reinrate_label, rx, ry, "#2e7d32", "square"))
        elif args.reinrate_csv:
        ...
        with average_csv.open("w", newline="") as handle:
            ...
            for label, xs, ys, _, _ in series:
                runs = ic if label == "Ideal" else mc if label == "Minstrel-HT" else reinrate_runs
