# 【中文注释总览】
# 本文件定义 DRL 智能体本身：DQNAgent、PolicyNetwork 和 ReinforceAgent。
# 当前 ns3ai.py 默认使用 ReinforceAgent(1, 8)：输入维度为 1，动作数为 8，对应 802.11n 单空间流 MCS0-MCS7。
#
# MDP 视角：PolicyNetwork 学习策略 pi(a|s)，输入通常是经过 log 缩放的 SNR，输出 8 个 MCS 动作概率。
# ReinforceAgent.update() 根据累积奖励 returns 和 log_prob 执行 REINFORCE 策略梯度更新。
import random
import torch
...
# 【DQN 经验池】保存 (state, action, reward, next_state, done)，用于备用 DQN 路径。
class ReplayBuffer:
...
# 【DQN 智能体】备用值函数方法，输出每个 MCS 动作的 Q 值。当前主流程默认使用 ReinforceAgent。
class DQNAgent(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.fc = nn.Sequential(
            # 【状态输入层】state_dim 来自 ns3ai.py，当前通常为 1，即 log(SNR+1)。
            nn.Linear(state_dim, 64),
            nn.ReLU(),
            nn.Linear(64, 64),
            nn.ReLU(),
            # 【动作输出层】action_dim=8，对应 8 个 802.11n MCS 动作。
    ...
    # 【动作选择】训练时 epsilon 探索/采样，评估时选择最大概率或最大 Q 值动作。
    ...
    # 【DQN 更新】根据 Bellman 目标更新 Q 网络；属于备用训练逻辑。
...
# 【策略网络】REINFORCE 使用的 pi(a|s)，输入状态，输出 8 个动作概率。
class PolicyNetwork(nn.Module):
    ...
    def forward(self, x):
        x = self.fc(x)
        # 【动作概率】softmax 将网络输出归一化为 MCS0-MCS7 的概率分布。
        return torch.softmax(x, dim=-1)

# 【REINFORCE 智能体】当前主流程使用该类，按策略梯度直接优化速率选择策略。
class ReinforceAgent:
    def __init__(self, state_size, action_size, lr=1e-4):
        self.policy = PolicyNetwork(state_size, action_size)
        self.optimizer = optim.Adam(self.policy.parameters(), lr=lr)
        # 【策略梯度缓存】保存采样动作的 log_prob，后续乘以回报形成 loss。
        self.saved_log_probs = []
        # 【奖励序列】由 ns3ai.py 根据 ns-3 返回状态计算后追加，用于计算折扣回报。
        self.rewards = []
        self.eps = 1e-8
    ...
    def choose_action(self, state, max_mcs = None, epsilon=0.3):
        ...
        if self.train_mode:
            ...
            if np.random.rand() < epsilon:
                ...
                m = Categorical(probs)
                # 【训练动作】从 Categorical(probs) 采样 MCS，保留探索性。
                action = m.sample()
                self.saved_log_probs.append(m.log_prob(action))
        else:
            # During evaluation, choose the action with the highest probability
            # 【推理动作】评估模式下选择概率最大的 MCS。
            action = torch.argmax(probs)
            
        return action.item()

    # 【REINFORCE 更新】按 G_t=sum gamma^k r_{t+k} 计算回报，用 -log pi(a|s)*G_t 做梯度下降。
