...
def find_project_root(start):
    """Locate the ns-3 root from either the legacy or packaged source layout."""
    for candidate in (start, *start.parents):
        if (candidate / "ns3").is_file() and (candidate / "scratch").is_dir():
            return candidate
    raise RuntimeError(f"Cannot find ns-3 project root from {start}")


ROOT = find_project_root(SOURCE_DIRECTORY)
LEGACY_REPRODUCTION_DIRECTORY = ROOT / "scratch/reproduction"
if str(LEGACY_REPRODUCTION_DIRECTORY) not in sys.path:
    sys.path.insert(0, str(LEGACY_REPRODUCTION_DIRECTORY))
import reproduction_reinrate_py  # noqa: E402

PLOTTING_DIRECTORY = (
    SOURCE_DIRECTORY
    if (SOURCE_DIRECTORY / "plot_training_history.py").is_file()
    else SOURCE_DIRECTORY.parent / "plotting"
)
...
DEFAULT_BE_MAX_AMPDU_SIZE = 0
AMPDU_OFF_MAX_REFERENCE_MBPS = 33.6
AMPDU_ON_MAX_REFERENCE_MBPS = 60.9


def ampdu_mode(be_max_ampdu_size):
    """Return a stable label for the requested best-effort A-MPDU setting."""
    if be_max_ampdu_size < 0:
        raise ValueError("BE_MaxAmpduSize must be non-negative")
    return "ampdu-off" if be_max_ampdu_size == 0 else "ampdu-on"


def max_reference_mbps(be_max_ampdu_size):
    """Return the calibrated MCS7 reference for the selected A-MPDU mode."""
    return (
        AMPDU_OFF_MAX_REFERENCE_MBPS
        if ampdu_mode(be_max_ampdu_size) == "ampdu-off"
        else AMPDU_ON_MAX_REFERENCE_MBPS
    )
...
def build_command(
    seed, simulation_time, start_distance, moving_speed, throughput_csv,
    be_max_ampdu_size=DEFAULT_BE_MAX_AMPDU_SIZE,
    decision_per_ampdu=False,
    scenario=1,
    random_walk_min_distance=0.0,
    random_walk_max_distance=24.0,
    random_walk_direction_interval=2.0,
):
    ampdu_mode(be_max_ampdu_size)
    ...
    return [
        ...
        f"--ns3::rl-rateWifiManager::DecisionPerAmpdu={'true' if decision_per_ampdu else 'false'}",
...
def run_episode(
    ...
    scenario_prefix = f"reproduction-scenario{scenario}"
    throughput_csv = RESULTS / f"{scenario_prefix}-{label}-seed{seed}.csv"
    decision_csv = RESULTS / f"{scenario_prefix}-{label}-decisions-seed{seed}.csv"
    ...
    process = subprocess.Popen(
        build_command(
            seed, simulation_time, start_distance, moving_speed, throughput_csv,
            be_max_ampdu_size, decision_per_ampdu, scenario,
            random_walk_min_distance, random_walk_max_distance,
            random_walk_direction_interval,
        ),
        cwd=ROOT,
        env=environment,
        text=True,
    ...
    try:
        while True:
            ...
            if active_action is not None:
                ...
                if packet_count < 1 or (not decision_per_ampdu and packet_count > PACKET_WINDOW):
                ...
                if collect_segments:
                    segments.append({
                        ...
                        "throughput_mbps": observation["throughput"],
                        "reward": observation["window_reward"],
                    })
            ...
            if print_every and window > 0 and window % print_every == 0:
        ...
        partial_windows = 0 if decision_per_ampdu else sum(count < PACKET_WINDOW for count in packet_counts)
        if (not decision_per_ampdu and
                (partial_windows > 1 or (partial_windows and packet_counts[-1] == PACKET_WINDOW))):
...
def plot_milestones(max_episode):
    ...
    subprocess.run(
        [
            sys.executable,
            str(PLOTTING_DIRECTORY / "plot_training_episodes.py"),
            "--input-glob",
...
def plot_history(history_csv):
    ...
    subprocess.run(
        [
            sys.executable,
            str(PLOTTING_DIRECTORY / "plot_training_history.py"),
