...
Flow (all units are *episodes*; one episode is 80 s in Scenario 1 or 40 s in
Scenario 2):
...
def baseline_command(
    ...
    be_max_ampdu_size, scenario, random_walk_min_distance,
    random_walk_max_distance, random_walk_direction_interval,
):
...
def run_test(final_policy, args, interface):
    ...
    auto_prefix = AUTO_PREFIX.replace("scenario1", f"scenario{args.scenario}")
    ...
    for index, seed in enumerate(seeds, start=1):
        ...
        rl = T.run_episode(
            ...
            random_walk_min_distance=args.random_walk_min_distance,
            random_walk_max_distance=args.random_walk_max_distance,
            random_walk_direction_interval=args.random_walk_direction_interval,
        )
        ...
        subprocess.run(
            baseline_command("ns3::IdealWifiManager", seed, args.simulation_time,
                             ...
                             args.be_max_ampdu_size, args.scenario,
                             args.random_walk_min_distance, args.random_walk_max_distance,
                             args.random_walk_direction_interval),
    ...
    subprocess.run(
        [sys.executable, "-B", str(T.PLOTTING_DIRECTORY / "result_figure.py"),
         ...
         "--reinrate-glob", str(T.RESULTS / f"reproduction-scenario{args.scenario}-{run_label_base}-test-{runs}run-run??-seed*.csv"),
         "--reinrate-label", "Window-20 REINFORCE",
         "--output", str(output_svg),
         "--average-csv", str(average_csv),
         "--title", f"Scenario {args.scenario}: {runs}-Run Frozen-Policy Average"],
...
def main():
    ...
    parser.add_argument("--scenario", type=int, choices=(1, 2), default=1,
                        help="1=1-41 m linear motion; 2=0-24 m 1D RandomWalk2d")
    parser.add_argument("--simulationTime", dest="simulation_time", type=float, default=None,
                        help="episode duration (default: scenario 1=80 s, scenario 2=40 s)")
    parser.add_argument("--startDistance", dest="start_distance", type=float, default=1.0)
    parser.add_argument("--movingSpeed", dest="moving_speed", type=float, default=None,
                        help="STA speed (default: scenario 1=0.5 m/s, scenario 2=3.0 m/s)")
    parser.add_argument("--randomWalkMinDistance", dest="random_walk_min_distance", type=float, default=0.0)
    parser.add_argument("--randomWalkMaxDistance", dest="random_walk_max_distance", type=float, default=24.0)
    parser.add_argument("--randomWalkDirectionInterval", dest="random_walk_direction_interval", type=float, default=2.0)
    ...
    if args.simulation_time is None:
        args.simulation_time = 40.0 if args.scenario == 2 else 80.0
    if args.moving_speed is None:
        args.moving_speed = 3.0 if args.scenario == 2 else 0.5
    if args.scenario == 2 and not (
        args.random_walk_min_distance <= args.start_distance <= args.random_walk_max_distance
    ):
        parser.error("Scenario 2 start distance must be inside the random-walk bounds")
    mode = T.ampdu_mode(args.be_max_ampdu_size)
    ...
    if args.scenario == 2:
        print(f"    场景2: 1D RandomWalk2d, {args.random_walk_min_distance:g}-{args.random_walk_max_distance:g} m, "
              f"{args.moving_speed:g} m/s, 每 {args.random_walk_direction_interval:g} s随机判定方向, "
              f"每轮 {args.simulation_time:g} s", flush=True)
