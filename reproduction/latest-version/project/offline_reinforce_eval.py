...
def main():
    ...
    for index, seed in enumerate(seeds, start=1):
        ...
        rl = run_episode(
            ...
            args.start_distance,
            args.moving_speed,
            f"offline-window20-episodic-reinforce-{mode}-{run_tag}",
            training=False,
            log_decisions=index == 1,
            be_max_ampdu_size=args.be_max_ampdu_size,
        )
        ideal_csv = RESULTS / f"{run_prefix}-test-{args.runs}run-ideal-run{index:02d}-seed{seed}.csv"
        minstrel_csv = RESULTS / f"{run_prefix}-test-{args.runs}run-minstrel-run{index:02d}-seed{seed}.csv"
        subprocess.run(
            baseline_command(
    ...
    output_svg = RESULTS / f"{run_prefix}-test-{args.runs}run-average.svg"
    average_csv = RESULTS / f"{run_prefix}-test-{args.runs}run-average.csv"
    subprocess.run(
        [
            sys.executable,
            "-B",
            str(ROOT / "scratch/reproduction/result_figure.py"),
            "--ideal-glob", str(RESULTS / f"{run_prefix}-test-{args.runs}run-ideal-run*-seed*.csv"),
            "--minstrel-glob", str(RESULTS / f"{run_prefix}-test-{args.runs}run-minstrel-run*-seed*.csv"),
            "--reinrate-glob", str(RESULTS / f"reproduction-scenario1-offline-window20-episodic-reinforce-{mode}-test-{args.runs}run-run??-seed*.csv"),
